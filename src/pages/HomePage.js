...
export default function HomePage({ addToCart, goToProductDetail }) {
  ...
  useEffect(() => {
    ...
    fetch('https://shop-api-0d5u.onrender.com/api/categories')
      .then(res => res.json())
      .then(data => {
    ...
    fetch('https://shop-api-0d5u.onrender.com/api/articles?populate=*')
      .then(res => res.json())
      .then(data => {
        setArticles(data.data.map(item => ({
          ...
          image: item.image ? `https://shop-api-0d5u.onrender.com${item.image.url}` : '👟'
        })));
        setLoading(false);
      })
      .catch(err => {
